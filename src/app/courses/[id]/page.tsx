...
export default function CourseDetailPage({ params }: PageProps) {
  ...
  return (
    <div className="flex-1 flex flex-col bg-[#f8f9fa] min-h-screen pb-20">
      {/* Header */}
      <header className="sticky top-0 z-30 w-full glass-panel border-b border-slate-200/50 backdrop-blur-md px-4 py-3 flex items-center gap-3">
        <Link href="/" className="p-1 rounded-full hover:bg-slate-100 text-slate-505 hover:text-slate-800 transition-all">
          <ChevronLeft className="h-5 w-5" />
        </Link>
        <div className="min-w-0 flex-1">
          <h1 className="font-extrabold text-sm text-slate-800 truncate">{course.title}</h1>
        </div>
      </header>
      ...
      <main className="flex-1 max-w-5xl mx-auto w-full flex flex-col md:grid md:grid-cols-3 gap-6 p-4">
        ...
        <div className="md:col-span-2 flex flex-col gap-4">
          ...
          <div className="relative aspect-[16/9] w-full bg-[#0f172a] rounded-2xl overflow-hidden border border-slate-200 shadow-lg">
            {isVideoUnlocked ? (
              <div className="w-full h-full flex flex-col items-center justify-center bg-slate-950 text-center p-4">
                ...
                <p className="text-slate-200 text-xs font-bold truncate max-w-xs sm:max-w-md">
                ...
                <span className="text-[9px] text-slate-500 mt-2">（模拟防录屏 HLS 鉴权通过）</span>
              </div>
            ) : (
              <div className="absolute inset-0 bg-slate-950/95 backdrop-blur-xs flex flex-col items-center justify-center p-6 text-center">
                ...
                <h3 className="text-sm font-bold text-white mb-2">本节课时需购买解锁</h3>
                ...
                <button
                  ...
                  disabled={buying}
                  className="flex items-center justify-center gap-2 px-6 py-3 rounded-full text-xs font-black bg-white text-slate-900 hover:bg-slate-100 active:scale-95 transition-all shadow-lg"
                >
          ...
          <div className="glass-card p-5 rounded-2xl">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3 mb-4">
              <h2 className="font-extrabold text-sm text-slate-800">课程介绍</h2>
              <span className="text-[10px] font-semibold text-slate-600 bg-slate-100 px-2.5 py-1 rounded-full flex items-center gap-0.5">
                <Sparkles className="h-3 w-3" />
                精品微课
              </span>
            </div>
            <div 
              className="prose text-xs sm:text-sm text-slate-650"
        ...
        <div className="glass-card p-4 rounded-2xl flex flex-col gap-3 h-fit">
          <h2 className="font-extrabold text-xs text-slate-400 uppercase tracking-wider border-b border-slate-100 pb-3 mb-1">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1.5 text-[9px] text-slate-400">
                      ...
                      {lesson.is_free_preview && (
                        <span className="text-blue-600 bg-blue-50 px-1 rounded-sm text-[8px] font-semibold">
                          试看
                        </span>
                      )}
                    </div>
          ...
          {!hasAccess && (
            <div className="mt-4 pt-4 border-t border-slate-100">
              <button
                ...
                onClick={handleBuyCourse}
                disabled={buying}
                className="w-full flex items-center justify-center gap-1.5 py-3 rounded-xl text-xs font-bold bg-slate-900 text-white hover:bg-slate-800 active:scale-95 transition-all shadow-md shadow-slate-900/10"
              >
                <CreditCard className="h-3.5 w-3.5" />
